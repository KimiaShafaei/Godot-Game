...
enum Tile { OBSTACLE = 2, START_POINT, END_POINT }
...
func _ready():
	...
	_astar.region = Rect2i(used_rect.position, used_rect.size)
	...
	for i in range(_astar.region.position.x, _astar.region.end.x):
		for j in range(_astar.region.position.y, _astar.region.end.y):
			...
			_astar.set_point_solid(pos.x, pos.y
			)
	
	for i in range(_astar.region.position.x, _astar.region.end.x):
		for j in range(_astar.region.position.y, _astar.region.end.y):
			var pos = Vector2i(i, j)
			var tile_coords = get_cell_atlas_coords(pos.x, pos.y)
			if tile_coords == Vector2i(Tile.OBSTACLE, 0):
